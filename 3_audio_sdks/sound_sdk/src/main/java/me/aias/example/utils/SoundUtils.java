...
public class SoundUtils {
    ...
    public static byte[] convertAsByteArray(byte[] source, AudioType targetType) {
        try {
            ...
            AudioInputStream ais = toStream(source);
...
// Intermediate conversion is supported
     ...
     */
    private static AudioInputStream toStream(byte[] bytes)
    ...
    /**
     * Converts an file to a byte array with the given sample length.
     */
    private static byte[] audioByteArrayBufferFile(File file, int lengthInSamples) {
        byte[] bytes = null;
        try {
            AudioInputStream audioStream = AudioSystem.getAudioInputStream(file);
            AudioFormat format = audioStream.getFormat();
            bytes = new byte[lengthInSamples * format.getSampleSizeInBits()];
            audioStream.read(bytes);
        } catch (IOException e) {
            throw new Error("Audio file should be present.");
        } catch (UnsupportedAudioFileException e) {
            throw new Error("Audio file format should be supported.");
        }
        return bytes;
    }

    /**
     * Converts an file to a float array with the given sample length.
     */
    private static float[] audioFloatArrayBufferFile(File file, int lengthInSamples) {
    ...
    /**
     * @return a silent buffer (all zeros), at sampleRate.
     */
    public static byte[] audioBufferSilence(int seconds, int sampleRate) {
//        final int sampleRate = 44100.0; 44.1kHz.
        final byte[] buffer = new byte[seconds * sampleRate * 2];
        return buffer;
    }
}