...
<script>
import { imagePoseForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imagePoseForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imagePoseForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
