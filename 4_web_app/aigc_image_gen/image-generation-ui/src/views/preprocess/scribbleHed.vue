...
<script>
import { imageScribbleHedForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/controlnet/bag.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/preprocess/imageScribbleHedForImageFile'
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageScribbleHedForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageScribbleHedForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
