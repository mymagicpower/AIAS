...
<script>
import { imageSegForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/controlnet/house.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageSegForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageSegForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
