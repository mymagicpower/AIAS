...
<script>
import { imageScribblePidiForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageScribblePidiForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageScribblePidiForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
