...
<script>
import { imageSoftEdgeHedForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageSoftEdgeHedForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageSoftEdgeHedForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
