    ...
    <el-row type="flex" justify="end" :gutter="20">
      <el-col :span="6">
        <el-input v-model="name" placeholder="请输入模版名称">
          <template slot="prepend">模版名称</template>
        </el-input>
      </el-col>
      <el-col :span="10">
        <el-button
...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      const pass = file.type === 'image/jpg' || 'image/jpeg' || 'image/png' || 'image/bmp'
      if (!pass) {
      ...
      this.labelData.forEach(element => {
        if (element.type === 'rectangle') {
          if (typeof element.field === 'undefined' || element.field === '') {
            pass = false
            this.$message.error('内容识别区字段名不能为空!')
          }
        }
      })
          ...
          for (let j = i + 1; j < this.labelData.length; j++) {
            ...
            if (element1.type === 'rectangle' && element2.type === 'rectangle') {
              if (element1.field === element2.field) {
                pass = false
                this.$message.error('内容识别区字段名不能重复!')
              }
            }
          }
        }
