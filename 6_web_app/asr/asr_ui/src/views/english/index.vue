...
<script>
import { enAsrForAudioUrl } from '@/api/inference'
...
export default {
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/inference/enAsrForAudioFile'
      return `${process.env.VUE_APP_BASE_API}/inference/enAsrForAudioFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      enAsrForAudioUrl(this.form).then(response => {
        this.fullscreenLoading = false
