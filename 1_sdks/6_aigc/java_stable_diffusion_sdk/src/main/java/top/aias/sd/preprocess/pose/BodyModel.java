...
import top.aias.sd.preprocess.ControlPool;
...
/**
 * OpenPose 姿态检测,身体关键点模型
 *
 * @author Calvin
 * @mail 179209347@qq.com
 * @website www.aias.top
 */
public final class BodyModel implements AutoCloseable {
    ...
    public BodyModel(int detect_resolution, int image_resolution, String modelPath, int poolSize, Device device)  {
    ...
    private Criteria<Image, Image> criteria() {
        ...
        Criteria<Image, Image> criteria =
                Criteria.builder()
                        ...
                        .optModelPath(Paths.get(modelPath)) // body.pt
                        .optDevice(device)
...
//                NDManager manager = ctx.getNDManager();
                ...
                for (int part = 0; part < 18; part++) {
                    ...
                    long[] arr1 = nonzero.get(new NDIndex(":,1")).toType(DataType.INT64, true).toLongArray();
                    long[] arr0 = nonzero.get(new NDIndex(":,0")).toType(DataType.INT64, true).toLongArray();
                    int peaksLength = arr1.length;
                    float[][] peaks_with_score_and_id = new float[peaksLength][4];
                    float[][] map_ori_arr = NDArrayUtils.floatNDArrayToArray(map_ori);
                    for (int i = 0; i < peaksLength; i++) {
                        peaks_with_score_and_id[i][0] = arr1[i];
                        peaks_with_score_and_id[i][1] = arr0[i];
                        peaks_with_score_and_id[i][2] = map_ori_arr[(int) arr0[i]][(int) arr1[i]]; // map_ori.get(arr0[i], arr1[i]).toFloatArray()[0];
                ...
                for (int k = 0; k < mapIdx.length; k++) {
                    ...
                    NDArray score_mid_0 = paf_avg.get(new NDIndex(":,:," + (idx[0] - 19)));
                    float[][] score_mid_arr_0 = NDArrayUtils.floatNDArrayToArray(score_mid_0);
                    NDArray score_mid_1 = paf_avg.get(new NDIndex(":,:," + (idx[1] - 19)));
                    float[][] score_mid_arr_1 = NDArrayUtils.floatNDArrayToArray(score_mid_1);
                ...
                NDArray ndArray = PoseUtils.draw_bodypose(manager, oriImg, canvas, candidate, newSubset);

                int[] hw = resize64(height, width, image_resolution);
                ndArray = NDImageUtils.resize(ndArray, hw[1], hw[0], Image.Interpolation.BILINEAR);
                Image img = ImageFactory.getInstance().fromNDArray(ndArray);
