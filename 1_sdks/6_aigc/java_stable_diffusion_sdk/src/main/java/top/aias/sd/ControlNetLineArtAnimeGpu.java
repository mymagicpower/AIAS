...
public final class ControlNetLineArtAnimeGpu {
    ...
    public static void main(String[] args) throws ModelException, IOException, TranslateException {
        ...
        try (StableDiffusionControlNetPipeline model = new StableDiffusionControlNetPipeline("H:\\models\\aigc\\sd_gpu\\", "controlnet_lineart_anime.pt", Device.gpu());
             LineArtAnimeModel detector = new LineArtAnimeModel(512, 512, modelPath, 1, Device.gpu())) {
            Image img = detector.predict(image);
            ImageUtils.saveImage(img, "lineart_anime.png", "build/output");
            Image result = model.generateImage(img, prompt, "", 25);
            ImageUtils.saveImage(result, "ctrlnet_lineart_anime_gpu.png", "build/output");
        }
