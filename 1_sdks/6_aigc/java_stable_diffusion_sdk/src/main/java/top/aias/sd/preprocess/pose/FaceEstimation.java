...
public final class FaceEstimation {
    ...
    public ArrayList<ArrayList<float[]>> faces(NDManager manager,FaceModel faceModel, NDArray oriImg, ArrayList<int[]> faces_list) throws TranslateException {
...
//                ImageUtils.saveImage(img, "hand_" + i + ".png", "build/output");
            ...
            ArrayList<float[]> peaks = estimation(manager, faceModel, subImg);
    ...
    public ArrayList<float[]> estimation(NDManager manager, FaceModel faceModel, NDArray face_img) throws TranslateException {
        int width = (int) face_img.getShape().get(0);
        int height = (int) face_img.getShape().get(1);

        NDArray ndArray = faceModel.predict(face_img);
