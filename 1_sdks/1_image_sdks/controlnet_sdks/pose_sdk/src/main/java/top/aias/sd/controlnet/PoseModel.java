 ...
 */
public final class PoseModel implements AutoCloseable {
    private int detect_resolution = 512;
    private int image_resolution = 512;
    private ZooModel<Image, NDList> model;
    private Device device;
    private PosePool pool;
    private String modelPath;
    private int poolSize;
    private final AtomicBoolean initialized = new AtomicBoolean(false);

    public PoseModel(int detect_resolution, int image_resolution, String modelPath, int poolSize, Device device) throws ModelException, IOException {
    ...
    public synchronized void ensureInitialized() {
        if (!initialized.get()) {
            ...
            this.pool = new PosePool(model, poolSize);
    ...
    public NDList predict(Image img) throws TranslateException {
        ensureInitialized();
        Predictor<Image, NDList> predictor = pool.getPredictor();
        NDList result = predictor.predict(img);
