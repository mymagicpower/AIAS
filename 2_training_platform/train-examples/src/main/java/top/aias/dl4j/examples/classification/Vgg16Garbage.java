 ...
 */
public class Vgg16Garbage {
    private static final Logger log = LoggerFactory.getLogger(Vgg16Garbage.class);
    private static String modelFilePath = new File(".").getAbsolutePath() + "/generated-models/";
    private static String modelFileName = modelFilePath + "VGG16_garbage.zip";
    // 训练参数
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 加载数据
        vgg16Model.loadData(new ClassPathResource("garbage").getFile(), 70);
        ...
        if (new File(modelFileName).exists()) {
            ...
            // 1. 构建模型
            ComputationGraph computationGraph = vgg16Model.build();
//            computationGraph = vgg16Model.loadModel(new File(modelFileName));
            ...
            for (int i = 0; i < nEpochs; i++) {
                ...
                log.info("*** epoch {} 完成 ***", i);
                // 3. 保存模型
                ModelSerializer.writeModel(computationGraph, modelFilePath + "VGG16_garbage_" + i + ".zip", true);
            }
        }
...
//        validationTestDataset(vgg16Model, testIter);
    ...
    private static void validationTestDataset(Vgg16Model vgg16Model, RecordReaderDataSetIterator test) {
        ...
        while (test.hasNext()) {
            ...
            ClassPrediction classPrediction = getPredictions(vgg16Model, image);
            System.out.println("预测结果:  " + classPrediction);
        }
    }

    private static ClassPrediction getPredictions(Vgg16Model vgg16Model, INDArray image) {
        // 获取模型预测结果
