 ...
 */
public class ResNet50Garbage {
    private static final Logger log = LoggerFactory.getLogger(ResNet50Garbage.class);
    private static String modelFilePath = new File(".").getAbsolutePath() + "/generated-models/";
    private static String modelFileName = modelFilePath + "ResNet50_Garbagex.zip";
    // 训练参数
    private static int batchSize = 8;
    private static int nEpochs = 6;
    // 训练数据标签数（分类数）
    protected static int nClasses = 6;
...
//        vgg16Model.loadData(trainDir, testDir);
        // 加载数据
        model.loadData(new ClassPathResource("garbage").getFile(), 70);
        ...
        if (new File(modelFileName).exists()) {
            ...
            for (int i = 0; i < nEpochs; i++) {
                ...
                log.info("*** epoch {} 完成 ***", i);
                // 3. 保存模型
                ModelSerializer.writeModel(computationGraph, modelFilePath + "ResNet50_Garbage_" + i + ".zip", true);
            }
        }
...
//        validationTestDataset(model, testIter);
    ...
    private static ClassPrediction getPredictions(ResNet50Model model, INDArray image) {
        ...
        INDArray argMax = predictions.argMax(1);
        int predictedClassIndex = argMax.getInt(0);
