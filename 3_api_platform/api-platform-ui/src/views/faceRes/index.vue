...
<script>
import { faceResForImageUrl } from '@/api/inference'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/assets/beauty.jpg',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/inference/faceResForImageFile'
      return `${process.env.VUE_APP_BASE_API}/api/img/faceResForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      faceResForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
