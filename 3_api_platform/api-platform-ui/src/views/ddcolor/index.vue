...
<script>
import { imageColorForImageUrl } from '@/api/inference'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/ap-images/audrey_hepburn.jpg',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/color/imageColorForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageColorForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
