...
<script>
import { animeSegForImageUrl } from '@/api/inference'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/assets/anime-girl.jpg',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      ...
      return `${process.env.VUE_APP_BASE_API}/api/seg/animeSegForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      animeSegForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
