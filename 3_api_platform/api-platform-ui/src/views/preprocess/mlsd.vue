...
<script>
import { imageMlsdForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/controlnet/mlsd.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/preprocess/imageSrForImageFile'
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageMlsdForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageMlsdForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
