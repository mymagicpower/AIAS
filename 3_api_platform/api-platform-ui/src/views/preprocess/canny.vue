...
<script>
import { imageCannyForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/controlnet/bird.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/preprocess/imageCannyForImageFile'
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageCannyForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageCannyForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
