...
<script>
import { imageDepthDptForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageDepthDptForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageDepthDptForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
