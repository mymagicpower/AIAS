...
<script>
import { imageNormalForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageNormalForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageNormalForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
