...
<script>
import { imageDepthMidasForImageUrl } from '@/api/preprocess'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/images/controlnet/toy.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      return `${process.env.VUE_APP_BASE_API}/api/preprocess/imageDepthMidasForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageDepthMidasForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
