...
<script>
import { imageSrForImageUrl } from '@/api/inference'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/assets/srgan.png',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/inference/imageSrForImageFile'
      return `${process.env.VUE_APP_BASE_API}/api/img/imageSrForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      imageSrForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
