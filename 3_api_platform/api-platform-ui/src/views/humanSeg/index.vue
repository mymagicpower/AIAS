...
<script>
import { humanSegForImageUrl } from '@/api/inference'
...
export default {
  ...
  methods: {
    upload() {
      ...
      return `${process.env.VUE_APP_BASE_API}/api/seg/humanSegForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      humanSegForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
