...
<script>
import { enAsrForLongAudioUrl } from '@/api/inference'
...
export default {
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/api/asr/enAsrForLongAudioFile'
      return `${process.env.VUE_APP_BASE_API}/api/asr/enAsrForLongAudioFile`
    },
    submitUpload() {
    ...
    beforeUpload(file) {
      ...
      if (!pass) {
        this.$message.error('音频文件应为 wav,mp3等!')
      }
      return pass
    },
    onSubmit() {
      this.fullscreenLoading = true
      enAsrForLongAudioUrl(this.form).then(response => {
        this.fullscreenLoading = false
