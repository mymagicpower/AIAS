package top.aias.platform.pool.trans;// 导入需要的包

import ai.djl.inference.Predictor;
import ai.djl.ndarray.NDList;
import ai.djl.repository.zoo.ZooModel;
import top.aias.platform.generate.LMOutput;
import top.aias.platform.translator.trans.DecoderTranslator;
 ...
 */
public class TextDecoderPool {
    private int poolSize;
    private ZooModel<NDList, NDList> model;
    private ArrayList<Predictor<NDList, LMOutput>> predictorList = new ArrayList<>();


    public TextDecoderPool(ZooModel<NDList, NDList> model, int poolSize) {
        this.poolSize = poolSize;
        this.model = model;

        for (int i = 0; i < poolSize; i++) {
            Predictor<NDList, LMOutput> predictor = model.newPredictor(new DecoderTranslator());
