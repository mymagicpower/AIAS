...
public class FinalSeg {
    ...
    public void viterbi(String sentence, List<String> tokens) {
        ...
        Map<Character, me.aias.jieba.Node> path = new HashMap<Character, me.aias.jieba.Node>();
        ...
        for (char state : states) {
            ...
            path.put(state, new me.aias.jieba.Node(state, null));
        }

        for (int i = 1; i < sentence.length(); ++i) {
            Map<Character, Double> vv = new HashMap<Character, Double>();
            v.add(vv);
            Map<Character, me.aias.jieba.Node> newPath = new HashMap<Character, me.aias.jieba.Node>();
            for (char y : states) {
                Double emp = emit.get(y).get(sentence.charAt(i));
                if (emp == null)
                    emp = MIN_FLOAT;
                me.aias.jieba.Pair<Character> candidate = null;
                ...
                newPath.put(y, new me.aias.jieba.Node(y, path.get(candidate.key)));
            }
            path = newPath;
        }
