...
public final class WordEncoderExample4 {
  ...
  public static void main(String[] args) throws IOException, ModelException, TranslateException {
    Path vocabPath = Paths.get("src/test/resources/w2v_sikuquanshu_dim300_vocab.txt");
    Path embeddingPath = Paths.get("src/test/resources/w2v_sikuquanshu_dim300.npy");

    WordEncoder encoder = new WordEncoder(vocabPath, embeddingPath);

    // 获取单词的特征值embedding
    float[] embedding1 = encoder.search("曰");
    logger.info("曰-特征值: " + Arrays.toString(embedding1));
    float[] embedding2 = encoder.search("云");
    logger.info("云-特征值: " + Arrays.toString(embedding1));
