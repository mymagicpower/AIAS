...
class ChatGLM2(torch.nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        self.model = (
            AutoModel.from_pretrained(model_name, trust_remote_code=True, torchscript=True).float().cpu()
        )
        self.config = {
            ...
            "return_last_logit": config.get("return_last_logit", True)}
...
while True:
    ...
    # torch.onnx.export(glmModel, (input_ids, position_ids, attention_mask, past_key_values), f'traced_chatglm2_6b_fp32_cpu.onnx',
    #                   export_params=True,  # 导出训练好的模型参数
    #                   verbose=10,  # debugmessage
    ...
    aaa = torch.topk(next_token_scores, top_k)
    bbb = aaa[0][..., -1, None]
