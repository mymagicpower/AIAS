...
<script>
import { autoTableInfoForImageUrl } from '@/api/table'
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        url: 'https://aias-home.oss-cn-beijing.aliyuncs.com/AIAS/OCR/images/document.jpeg',
        result1: '',
        result2: '',
        base64Img: ''
      }
  ...
  methods: {
    upload() {
      // return window.g.Base_URL + '/inference/autoTableInfoForImageFile'
      return `${process.env.VUE_APP_BASE_API}/table/autoTableInfoForImageFile`
    },
    submitUpload() {
    ...
    onSubmit() {
      this.fullscreenLoading = true
      autoTableInfoForImageUrl(this.form).then(response => {
        this.fullscreenLoading = false
